...
export default function ScheduleGrid() {
  ...
  return (
    <div className="flex flex-row border border-r-0 border-slate-300 rounded shadow overflow-x-auto overflow-y-auto">
      {DAY_ORDER.map((d, di) => (
        <div key={d} className="border-r border-slate-300 min-w-28 md:min-w-38">
          <div className="px-2 py-1 font-semibold border-b border-slate-300 bg-slate-50 text-slate-800">
          ...
          <ul className="p-1 md:p-2 space-y-2">
            ...
            {byIndexDay[di].map((blk, i) => (
              <li
                key={i}
                className="border border-slate-200 shadow rounded min-h-20 md:min-h-26 p-1 md:p-2 text-xs md:text-sm text-slate-900"
              >
